# -*- coding: utf-8 -*-
"""Punto 3 corregido.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1GfPuF0jon5O3FtT74XedsQS5_KVPwE2i
"""
...
# Normalizar para que ∫ phi^2 dx = 1 (descomenta si quieres forzarlo)
phi0 /= np.sqrt(max(1e-16, trapz_periodic(phi0 * phi0)))
# Si prefieres NO normalizar, comenta la línea anterior.
...
explicacion_path = out_dir / f"{nombre}_explicacion.txt"

# gráfica inicial vs final
plt.figure()
plt.plot(x, phi0, label="t=0")
plt.plot(x, phi, label=f"t={t:.2f}")
plt.xlabel("x"); plt.ylabel("u(t,x)")
plt.title("KdV (dif. finitas, periódicas)")
...
# blit=False para que el título se actualice correctamente
anim = animation.FuncAnimation(
    fig, animate, init_func=init,
    frames=len(snapshots), interval=180, blit=False
)
...
note = dedent(f"""
    Simulación KdV (dif. finitas, condiciones periódicas)
...
with open(explicacion_path, "w", encoding="utf-8") as f:
    f.write(note)
